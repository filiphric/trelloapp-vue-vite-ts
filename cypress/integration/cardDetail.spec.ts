...
describe('card detail', () => {
  ...
  beforeEach( () => {
    ...
    cy.addBoardApi('new board').addListApi({name: 'new list'}).addCardApi({name: 'new card'});
  ...
  it('uploads a file', () => {
      ...
      cy.get('[data-cy="notification-message"]').should('exist').and('contain.text', 'File was sucessfully uploaded')
      cy.tick(4000);
  ...
  it('shows error message when uploading a file fails', () => {
      ...
      cy.visit(`/board/${Cypress.env('boards')[0].id}?card=${Cypress.env('cards')[0].id}`);
      cy.get('[data-cy="upload-image"]').attachFile('cypressLogo.png', { subjectType: 'drag-n-drop' })
      cy.get('[data-cy="notification-message"]').should('exist').and('contain.text', 'There was an error uploading file')
      cy.tick(4000);
      cy.get('[data-cy="notification-message"]').should('not.exist')
  ...
  it('changes card date', () => {
    ...
    cy.wait('@updateCard').its('response.body.deadline').should('eq', '2021-08-29')
  ...
  it('copies card properties', () => {
    ...
    cy.get('[data-cy="notification-message"]').should('exist').and('contain.text', 'Card info copied to clipboard')
    cy.tick(4000);
    cy.get('[data-cy="notification-message"]').should('not.exist')
  ...
  it('renames a card', () => {
    ...
    cy.get('[data-cy="card-title"]').should('have.value', 'new name')
    cy.get('[data-cy="notification-message"]').should('exist').and('contain.text', 'Card was renamed')
    cy.tick(4000);
    cy.get('[data-cy="notification-message"]').should('not.exist')
  ...
  it('deletes a card', () => {
    ...
    cy.get('[data-cy="notification-message"]').should('exist').and('contain.text', 'Card was deleted')
    cy.tick(4000);
    cy.get('[data-cy="notification-message"]').should('not.exist')
