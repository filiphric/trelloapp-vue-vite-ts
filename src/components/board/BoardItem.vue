<template>
  <router-link custom v-slot="{ navigate }" :to="{ path: '/board/' + board.id }">
    ...
    >
      ...
      <div data-cy="star" v-show="showStar" @click.stop="updateBoardStarred(board)" class="justify-self-end self-start">
        <Star class="w-5 h-5 col-span-1" :class="[board.starred ? 'fill-current text-yellow-300' : 'stroke-current text-white']" />
      </div>
    </div>
  </router-link>
</template>
